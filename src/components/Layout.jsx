...
import { 
  Home, FileText, MessageSquare, Shield, Activity, LogOut, Menu, X, Zap
} from 'lucide-react'
import { useState } from 'react'

export default function Layout() {
  const { user, logout } = useAuth()
  const { toggleInspector, isInspectorOpen, hasNewLogs } = usePrivacy()
          ...
          <div className="flex justify-between items-center h-16">
            ...
            <div className="flex items-center space-x-2 md:space-x-4">
              <button
                ...
                className={`flex items-center px-3 py-2 text-sm font-medium rounded-md transition-all border relative ${
                  isInspectorOpen 
              ...
              >
                <div className="relative">
                  <Zap className={`h-4 w-4 md:mr-2 ${isInspectorOpen ? 'animate-pulse text-accent-600' : 'text-gray-400'}`} />
                  {hasNewLogs && !isInspectorOpen && (
                ...
                <span className="hidden md:inline">Privacy Inspector</span>
              </button>
